-- River Hub — scope classify_spills by asset and/or year.
-- Historical backfill pushed spill_events to tens of thousands of rows; the unscoped RPC
-- (per-event rainfall subqueries returning every event) no longer scales. Add optional
-- p_asset / p_year filters so the asset page and a year-filtered dry-spills page stay fast.

drop function if exists classify_spills(int, numeric);

create or replace function classify_spills(
...
as $$
  with ev as (
    ...
    where (p_asset is null or e.asset_id = p_asset)
      and (p_year  is null or extract(year from e.event_start) = p_year)
  )
  select
...
grant execute on function classify_spills(int, numeric, uuid, int) to authenticated;
